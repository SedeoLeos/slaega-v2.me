...
export async function GET(req: NextRequest) {
  ...
  if (admin) {
    ...
    return NextResponse.json(await penseeRepository.getAll());
  }
  return NextResponse.json(await penseeRepository.getPublished());
}
...
export async function POST(req: NextRequest) {
  ...
  const created = await penseeRepository.create(body);
...
export async function PUT(req: NextRequest) {
  ...
  const body = await req.json().catch(() => null);
  if (!body) return NextResponse.json({ message: "Body invalide" }, { status: 400 });
  const updated = await penseeRepository.update(id, body);
...
export async function DELETE(req: NextRequest) {
  ...
  const id = req.nextUrl.searchParams.get("id");
  if (!id) return NextResponse.json({ message: "ID manquant" }, { status: 400 });
  const ok = await penseeRepository.delete(id);
