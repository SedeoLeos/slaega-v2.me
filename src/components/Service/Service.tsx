  ...
  return (
    <div className='bg-neutral-500 min-h-screen flex justify-center relative'>
      ...
      <div className='w-full max-w-content text-white py-20 px-10 lg:px-20 flex flex-col gap-20 justify-center items-center relative'>
        ...
        <StaggerContainer className='grid md:grid-cols-2 xl:grid-cols-3 max-w-[1044px] gap-7'>
          {items.map((service, index) => (
            <StaggerItem key={index}>
              <div className={`self-center place-self-center gap-5 flex flex-col p-7 border-stone-900 relative ${index === 3 ? 'md:col-span-2 md:max-w-full md:h-full' : 'md:max-w-80'}`}>
                ...
                <p className='text-gray-200 z-[2]'>{service.description}</p>
                <div className='bg-gradient-to-br from-zinc-900/50 to-zinc-950/100 mix-blend-plus-lighter absolute top-0 left-0 right-0 bottom-0 z-0 border-stone-900 border-2 rounded-xl' />
              </div>
            </StaggerItem>
          ))}
        </StaggerContainer>
